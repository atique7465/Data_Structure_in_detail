...
void Queue::push(ll v)
{
    if(frnt==-1 && rear==-1)
        frnt=rear=0;
    else if((rear+1)%oldsize==frnt)
    {   /// circular array technique used here to push new element to reduce memory waste.
        ll newsize=(rear-frnt+1)*2;
        ll *newa=new ll[newsize]; /// make a new array.
        ll i=0;
        ll j=frnt;
        while(j<=rear)
        {
            newa[i]=a[j%oldsize]; /// copy elements of previous array.
            i++;
            j++;
        }
        delete [] a;
        ...
        frnt=0;
        rear=i;
        oldsize=newsize; /// updadate size, front, rear accordingly.
    }
    else
        rear++;
    a[rear%oldsize]=v;
